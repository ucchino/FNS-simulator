...
NumeroNodi = input(prompt); %number of nodes

prompt='Please input the simulation duration (ms):';
DurataSimulazione = input(prompt); %duration of the simulation
...
switch modality
    case 1
        for ii = (1:(NumeroNodi))
            ...
            [LFP(ii).signal,edges] = histcounts(LFP(ii).contributions(:,1),(0:1:DurataSimulazione));

            % here we generate the related FFTs
            LFP(ii).FFTtot = abs(fft(LFP(ii).signal)/DurataSimulazione);
            LFP(ii).FFT = 2*LFP(ii).FFTtot(1:DurataSimulazione/2+1);

            f = 1000 *(0:(DurataSimulazione/2))/DurataSimulazione; % sampling freq. = 1000
        ...
        % [b_time, b_node, b_neuron, initial internal state, reached internal state]
            ...
            for jj = (1:(1+max(LFP(ii).sorted_contributions(:,3)))) % for each neuron of the node
                ...
                if ~isempty(temp) % this if is to process only the output of neurons that produced spikes   
                    ...
                    temp4 = zeros(1, DurataSimulazione);
                    temp4(1, 1:size (temp3,2))=temp3;
                    LFP(ii).time_series_collect (jj,3) = {temp4(1,1:DurataSimulazione)};
                else
                    LFP(ii).time_series_collect (jj,3) = {zeros(1,DurataSimulazione)};
                end
            end
...
figure ()
for ii = (1:NumeroNodi)
    ax(ii) = subplot(NumeroNodi,1,ii);
    plot (smooth(LFP(ii).signal','lowess')); xlim([0 DurataSimulazione]); ylabel(['N_' num2str(ii-1)]); % Smoothed
