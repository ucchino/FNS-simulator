...
DurataSimulazione = input(prompt);
 
edges = (0:1:DurataSimulazione); %for a resolution of 1 ms in MUA signal generation
...
for ii = (1:NumeroNodi)
    ...
    MUA(ii).FFTtot = abs(fft(MUA(ii).signal)/DurataSimulazione);
    MUA(ii).FFT = 2*MUA(ii).FFTtot(1:DurataSimulazione/2+1);
...
figure ()
for ii = (1:NumeroNodi)
    ax(NumeroNodi) = subplot(NumeroNodi,1,ii);
